...
CIronSightDialog::CIronSightDialog(vgui::Panel *parent) : BaseClass(parent, "IronSightDialog")
{
	...
	m_pWeaponScriptText = new vgui::TextEntry( this, "WeaponScriptText" );
	m_pWeaponScriptText->AddActionSignalTarget( this );
	m_pWeaponScriptText->SetMultiline( true );

	m_pCheckOverride = new vgui::CheckButton( this, "CheckOverride", "Ironsight override" );
	m_pCheckOverride->AddActionSignalTarget( this );
	...
	m_pCheckOverride->SetSelected( ironsight_override.GetBool() );

	InitActiveWeaponValues();
}

void CIronSightDialog::InitActiveWeaponValues()
{
	// do not override the convars if we already have some changes
	if ( !ironsight_override.GetBool() )
	{
		sscanf( ironsight_position.GetString(), "%f %f %f", &m_vecPosBackup.x, &m_vecPosBackup.y, &m_vecPosBackup.z );
		sscanf( ironsight_angle.GetString(), "%f %f %f", &m_vecRotBackup.x, &m_vecRotBackup.y, &m_vecRotBackup.z );
		float flFOV = m_pSliderFOV->GetValue();
		float flPivotForward = m_pSliderPivotForward->GetValue();
		float flLooseness = m_pSliderLooseness->GetValue();
		if ( GetActiveWeapon() )
		{
			WEAPON_FILE_INFO_HANDLE	hWpnInfo = LookupWeaponInfoSlot( GetActiveWeapon()->GetClassname() );
			if ( hWpnInfo != GetInvalidWeaponInfoHandle() )
			{
				CCSWeaponInfo* pWeaponInfo = dynamic_cast<CCSWeaponInfo*>(GetFileWeaponInfoFromHandle( hWpnInfo ));
				if ( pWeaponInfo )
				{
					m_vecPosBackup = pWeaponInfo->m_vecIronsightEyePos;
					m_vecRotBackup = Vector( pWeaponInfo->m_angIronsightPivotAngle.x, pWeaponInfo->m_angIronsightPivotAngle.y, pWeaponInfo->m_angIronsightPivotAngle.z );
					flFOV = pWeaponInfo->m_flIronsightFOV;
					flPivotForward = pWeaponInfo->m_flIronsightPivotForward;
					flLooseness = pWeaponInfo->m_flIronsightLooseness;
				}
			}
		}

		m_pSliderFOV->SetValue( flFOV );
		m_pSliderPivotForward->SetValue( flPivotForward );
		m_pSliderLooseness->SetValue( flLooseness * 100.0f );
	}
}
...
CIronSightDialog::~CIronSightDialog()
{
	...
	delete m_pWeaponScriptText;

	delete m_pCheckOverride;
}
...
void CIronSightDialog::OnMessage( const KeyValues *pParams, vgui::VPANEL fromPanel )
{
	if ( !Q_strcmp( "SliderMoved", pParams->GetName() ) )
	{
		...
		char szWeaponScriptText[512];
		V_snprintf(szWeaponScriptText, 512, "\t\"IronsightEyePos\"\t\t\t\"%s\"\n\t\"IronsightPivotAngle\"\t\t\"%s\"\n\t\"IronsightFOV\"\t\t\t\t\"%s\"\n\t\"IronsightPivotForward\"\t\t\"%s\"\n\t\"IronsightLooseness\"\t\t\"%f\"",
		...
		m_pWeaponScriptText->SetText( szWeaponScriptText );
...
CON_COMMAND_F( ironsight_editor, "Edit ironsights.", FCVAR_DONTRECORD )
{
