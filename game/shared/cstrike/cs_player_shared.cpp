...
bool CCSPlayer::CanPlayerBuy( bool display )
{
	...
	if ( CSGameRules()->m_bCTCantBuy && ( GetTeamNumber() == TEAM_CT ) )
	{
		if ( display == true )
	...
	if ( CSGameRules()->m_bTCantBuy && ( GetTeamNumber() == TEAM_TERRORIST ) )
	{
		if ( display == true )
	...
	if ( CSGameRules()->IsBuyTimeElapsed() && !CanBuyDuringImmunity() )
	{
		if ( display == true )
		{
			char strBuyTime[16];
